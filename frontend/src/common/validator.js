...
const rules = {
  ...
  email: {
    rule: (value) => {
      ...
      return value ? emailRegex.test(String(value).toLocaleLowerCase()) : true;
    },
...
/**
 * Валидирует одно поле и записывает ошибку в validations[key].
 * @returns {boolean} true, если поле валидно
 */
export const validateSingleField = (key, value, validations) => {
  if (!validations[key]) return true;
  validations[key].error = validate(value, validations[key].rules);
  return !validations[key].error;
};
